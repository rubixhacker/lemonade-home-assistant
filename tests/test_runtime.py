...
class LemonadeImageResultTest(unittest.TestCase):
    def test_generate_image_invokes_client_and_returns_typed_result(self) -> None:
        ...
        self.assertIsInstance(result, image_result_module.DecodedImage)
        self.assertEqual(b"image", result.image.image_bytes)
        self.assertEqual("image/jpeg", result.image.mime_type)
        self.assertEqual("jpg", result.image.extension)
...
class RuntimeSetupTest(unittest.IsolatedAsyncioTestCase):
    ...
    def test_strings_define_options_and_profile_subentry_translations(self) -> None:
        ...
        self.assertEqual(
            ...
            subentries[SUBENTRY_TYPE_CONVERSATION]["initiate_flow"]["user"],
        )
        self.assertEqual(
            "Add AI task profile",
    ...
    def test_direct_service_requests_parse_service_call_data_once(self) -> None:
        ...
        self.assertEqual(ReturnRawImageResponse(), image_request.intent)
        with self.assertRaises(FrozenInstanceError):
        ...
        self.assertEqual(
            ProduceImageArtifact(filename=None), saved_image_request.intent
        )

        named_saved_image_request = GenerateImageRequest.from_service_call(
            SimpleNamespace(
                data={
                    ATTR_PROMPT: "Draw",
                    ATTR_SAVE: True,
        ...
        self.assertEqual(
            ProduceImageArtifact(filename="lemon.png"),
            named_saved_image_request.intent,
        )
    ...
    def test_custom_component_runtime_translations_match_strings(self) -> None:
        strings = json.loads(Path("custom_components/lemonade/strings.json").read_text())
