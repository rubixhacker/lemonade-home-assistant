...
class LemonadeAITaskEntity(ai_task.AITaskEntity):
    ...
    async def _async_generate_data(self, task: Any, chat_log: Any) -> Any:
        ...
        data = await profile_chat.async_generate_ai_task_profile_data(
            entry=self.entry,
            profile=self.profile,
    ...
    async def _async_generate_image(self, task: Any, chat_log: Any) -> Any:
        ...
        if not isinstance(generated, DecodedImage):
            raise HomeAssistantError(AI_TASK_NO_IMAGE_ERROR)
        image_result = generated.image
